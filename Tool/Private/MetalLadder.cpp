...
CMetalLadder::CMetalLadder(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CTool_Object(pDevice, pContext)
{
}

CMetalLadder::CMetalLadder(const CMetalLadder& rhs)
    : CTool_Object(rhs)
{
}

HRESULT CMetalLadder::Initialize_Prototype()
{
    return S_OK;
}

HRESULT CMetalLadder::Initialize(void* pArg)
{
...
void CMetalLadder::Tick(_float fTimeDelta)
{
...
void CMetalLadder::Late_Tick(_float fTimeDelta)
{
...
HRESULT CMetalLadder::Add_Components()
{
    /* Com_Renderer */
    ...
    if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Model_MetalLadder"),
...
HRESULT CMetalLadder::Bind_ShaderResources()
{
...
CMetalLadder* CMetalLadder::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CMetalLadder* pInstance = new CMetalLadder(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        Safe_Release(pInstance);
        MSG_BOX("Failed to Create : CMetalLadder");
    }

    return pInstance;
}

CMetalLadder* CMetalLadder::Clone(void* pArg)
{
    CMetalLadder* pInstance = new CMetalLadder(*this);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        Safe_Release(pInstance);
        MSG_BOX("Failed to Clone : CMetalLadder");
    }

    return pInstance;
}

void CMetalLadder::Free()
{
    __super::Free();
