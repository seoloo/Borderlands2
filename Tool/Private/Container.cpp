...
HRESULT CContainer::Add_Components()
{
    ...
    if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Model_Container"),
...
CContainer* CContainer::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    ...
    if (FAILED(pInstance->Initialize_Prototype()))
    {
        ...
        MSG_BOX("Failed to Create : CContainer");
    }

    return pInstance;
}

CContainer* CContainer::Clone(void* pArg)
{
    ...
    {
        Safe_Release(pInstance);
        MSG_BOX("Failed to Clone : CContainer");
    }
