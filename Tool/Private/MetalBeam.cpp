...
CMetalBeam::CMetalBeam(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CTool_Object(pDevice, pContext)
{
}

CMetalBeam::CMetalBeam(const CMetalBeam& rhs)
    : CTool_Object(rhs)
{
}

HRESULT CMetalBeam::Initialize_Prototype()
{
    return S_OK;
}

HRESULT CMetalBeam::Initialize(void* pArg)
{
...
void CMetalBeam::Tick(_float fTimeDelta)
{
...
void CMetalBeam::Late_Tick(_float fTimeDelta)
{
...
HRESULT CMetalBeam::Add_Components()
{
    /* Com_Renderer */
    ...
    if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Model_MetalBeam"),
...
HRESULT CMetalBeam::Bind_ShaderResources()
{
...
CMetalBeam* CMetalBeam::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CMetalBeam* pInstance = new CMetalBeam(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        Safe_Release(pInstance);
        MSG_BOX("Failed to Create : CMetalBeam");
    }

    return pInstance;
}

CMetalBeam* CMetalBeam::Clone(void* pArg)
{
    CMetalBeam* pInstance = new CMetalBeam(*this);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        Safe_Release(pInstance);
        MSG_BOX("Failed to Clone : CMetalBeam");
    }

    return pInstance;
}

void CMetalBeam::Free()
{
    __super::Free();
