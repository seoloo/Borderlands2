...
CCrate::CCrate(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CTool_Object(pDevice, pContext)
{
}

CCrate::CCrate(const CCrate& rhs)
    : CTool_Object(rhs)
{
}

HRESULT CCrate::Initialize_Prototype()
{
    return S_OK;
}

HRESULT CCrate::Initialize(void* pArg)
{
...
void CCrate::Tick(_float fTimeDelta)
{
...
void CCrate::Late_Tick(_float fTimeDelta)
{
...
HRESULT CCrate::Add_Components()
{
    ...
    if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Model_Crate"),
...
HRESULT CCrate::Bind_ShaderResources()
{
...
CCrate* CCrate::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CCrate* pInstance = new CCrate(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        Safe_Release(pInstance);
        MSG_BOX("Failed to Create : CCrate");
    }

    return pInstance;
}

CCrate* CCrate::Clone(void* pArg)
{
    CCrate* pInstance = new CCrate(*this);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        Safe_Release(pInstance);
        MSG_BOX("Failed to Clone : CCrate");
    }
