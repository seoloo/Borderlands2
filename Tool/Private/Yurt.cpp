...
CYurt::CYurt(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CTool_Object(pDevice, pContext)
{
}

CYurt::CYurt(const CYurt& rhs)
    : CTool_Object(rhs)
{
}

HRESULT CYurt::Initialize_Prototype()
{
    return S_OK;
}

HRESULT CYurt::Initialize(void* pArg)
{
...
void CYurt::Tick(_float fTimeDelta)
{
...
void CYurt::Late_Tick(_float fTimeDelta)
{
...
HRESULT CYurt::Add_Components()
{
    ...
    if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Model_Yurt"),
...
HRESULT CYurt::Bind_ShaderResources()
{
...
CYurt* CYurt::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CYurt* pInstance = new CYurt(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        Safe_Release(pInstance);
        MSG_BOX("Failed to Create : CYurt");
    }

    return pInstance;
}

CYurt* CYurt::Clone(void* pArg)
{
    CYurt* pInstance = new CYurt(*this);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        Safe_Release(pInstance);
        MSG_BOX("Failed to Clone : CYurt");
    }
