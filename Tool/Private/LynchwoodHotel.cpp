...
CLynchwoodHotel::CLynchwoodHotel(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CTool_Object(pDevice, pContext)
{
}

CLynchwoodHotel::CLynchwoodHotel(const CLynchwoodHotel& rhs)
    : CTool_Object(rhs)
{
}

HRESULT CLynchwoodHotel::Initialize_Prototype()
{
    return S_OK;
}

HRESULT CLynchwoodHotel::Initialize(void* pArg)
{
...
void CLynchwoodHotel::Tick(_float fTimeDelta)
{
...
void CLynchwoodHotel::Late_Tick(_float fTimeDelta)
{
...
HRESULT CLynchwoodHotel::Render()
{
...
void CLynchwoodHotel::Apply_Tool()
{
...
void CLynchwoodHotel::Key_Input()
{
    CGameInstance* pGameInstance = CGameInstance::Get_Instance();
    Safe_AddRef(pGameInstance);
...
HRESULT CLynchwoodHotel::Add_Components()
{
    /* Com_Renderer */
    ...
    if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Model_LynchwoodHotel"),
...
HRESULT CLynchwoodHotel::Bind_ShaderResources()
{
...
CLynchwoodHotel* CLynchwoodHotel::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CLynchwoodHotel* pInstance = new CLynchwoodHotel(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        Safe_Release(pInstance);
        MSG_BOX("Failed to Create : CLynchwoodHotel");
    }

    return pInstance;
}

CLynchwoodHotel* CLynchwoodHotel::Clone(void* pArg)
{
    CLynchwoodHotel* pInstance = new CLynchwoodHotel(*this);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        Safe_Release(pInstance);
        MSG_BOX("Failed to Clone : CLynchwoodHotel");
    }

    return pInstance;
}

void CLynchwoodHotel::Free()
{
    __super::Free();
