...
CBedGarbage::CBedGarbage(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CTool_Object(pDevice, pContext)
{
}

CBedGarbage::CBedGarbage(const CBedGarbage& rhs)
    : CTool_Object(rhs)
{
}

HRESULT CBedGarbage::Initialize_Prototype()
{
    return S_OK;
}

HRESULT CBedGarbage::Initialize(void* pArg)
{
...
void CBedGarbage::Tick(_float fTimeDelta)
{
...
void CBedGarbage::Late_Tick(_float fTimeDelta)
{
...
HRESULT CBedGarbage::Add_Components()
{
    /* Com_Renderer */
    ...
    if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Model_BedGarbage"),
...
HRESULT CBedGarbage::Bind_ShaderResources()
{
...
CBedGarbage* CBedGarbage::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CBedGarbage* pInstance = new CBedGarbage(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        Safe_Release(pInstance);
        MSG_BOX("Failed to Create : CBedGarbage");
    }

    return pInstance;
}

CBedGarbage* CBedGarbage::Clone(void* pArg)
{
    CBedGarbage* pInstance = new CBedGarbage(*this);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        Safe_Release(pInstance);
        MSG_BOX("Failed to Clone : CBedGarbage");
    }

    return pInstance;
}

void CBedGarbage::Free()
{
    __super::Free();
