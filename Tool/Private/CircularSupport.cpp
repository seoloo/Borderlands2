...
CCircularSupport::CCircularSupport(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CTool_Object(pDevice, pContext)
{
}

CCircularSupport::CCircularSupport(const CCircularSupport& rhs)
    : CTool_Object(rhs)
{
}

HRESULT CCircularSupport::Initialize_Prototype()
{
    return S_OK;
}

HRESULT CCircularSupport::Initialize(void* pArg)
{
...
void CCircularSupport::Tick(_float fTimeDelta)
{
...
void CCircularSupport::Late_Tick(_float fTimeDelta)
{
...
HRESULT CCircularSupport::Render()
{
...
void CCircularSupport::Apply_Tool()
{
...
void CCircularSupport::Key_Input()
{
    CGameInstance* pGameInstance = CGameInstance::Get_Instance();
    Safe_AddRef(pGameInstance);
...
HRESULT CCircularSupport::Add_Components()
{
    /* Com_Renderer */
    ...
    if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Model_CicularSupport"),
...
HRESULT CCircularSupport::Bind_ShaderResources()
{
...
CCircularSupport* CCircularSupport::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CCircularSupport* pInstance = new CCircularSupport(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        Safe_Release(pInstance);
        MSG_BOX("Failed to Create : CCircularSupport");
    }

    return pInstance;
}

CCircularSupport* CCircularSupport::Clone(void* pArg)
{
    CCircularSupport* pInstance = new CCircularSupport(*this);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        Safe_Release(pInstance);
        MSG_BOX("Failed to Clone : CCircularSupport");
    }

    return pInstance;
}

void CCircularSupport::Free()
{
    __super::Free();
