...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
    {
        SetCursorPos(g_iWinSizeX * 0.5f, g_iWinSizeY * 0.5f);  // 화면 가운데로 초기화
        initMousPos.x = g_iWinSizeX * 0.5f;
        initMousPos.y = g_iWinSizeY * 0.5f;
    } 
        break;
    case WM_MOUSEMOVE:
    {
        int centerX = initMousPos.x;
        int centerY = initMousPos.y;
        SetCursorPos(centerX + 320, centerY + 180);
        ShowCursor(false);
    }
        break;
    case WM_PAINT:
