...
BEGIN(Client)
class CGoliath3 final : public CLandObject
{
public:
	...
	enum STATE{ STATE_APPEAR, STATE_IDLE, STATE_WALK, STATE_ATTACK, STATE_DEAD, STATE_END};
	enum MOVEDIR { MOVE_FORWARD, MOVE_BACKWARD, MOVE_RIGHT, MOVE_LEFT, MOVE_END };

private: 
	CGoliath3(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CGoliath3(const CGoliath3& rhs);
	virtual ~CGoliath3() = default;
...
public:
	static CGoliath3* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
