...
CMordecai::CMordecai(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CLandObject(pDevice, pContext)
{
}

CMordecai::CMordecai(const CMordecai & rhs)
	: CLandObject(rhs)
{
}

HRESULT CMordecai::Initialize_Prototype()
{
...
HRESULT CMordecai::Initialize(void * pArg)
{
	...
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMVectorSet(13.f, 0.f, 11.f, 1.f));

	return S_OK;
}

void CMordecai::Tick(_float fTimeDelta)
{
...
void CMordecai::Late_Tick(_float fTimeDelta)
{
...
HRESULT CMordecai::Add_Components()
{
	/* Com_Renderer */
	...
	/* Com_Model */
	if (FAILED(__super::Add_Component(g_CurLevel, TEXT("Prototype_Component_Model_Mordecai"),
...
HRESULT CMordecai::Bind_ShaderResources()
{
...
CMordecai * CMordecai::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CMordecai*	pInstance = new CMordecai(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Created : CMordecai");
	}

	return pInstance;
}

CGameObject * CMordecai::Clone(void * pArg)
{
	CMordecai*	pInstance = new CMordecai(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Cloned : CMordecai");
	}
