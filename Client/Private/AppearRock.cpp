...
CAppearRock::CAppearRock(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CGameObject(pDevice, pContext)
{
}

CAppearRock::CAppearRock(const CAppearRock& rhs)
    : CGameObject(rhs)
{
}

HRESULT CAppearRock::Initialize_Prototype()
{
...
HRESULT CAppearRock::Initialize(void* pArg)
{
	...
	m_Desc.iNumInstance = 1000;
	m_Desc.fLifeTime = 100;
	m_Desc.vPivotPos = Desc.vPivotPos;
	m_Desc.vRange = _float3(10.f, 0.f, 10.f);
...
void CAppearRock::Tick(_float fTimeDelta)
{
	...
	{
		fHeight = (m_fTimer * m_fTimer * -9.8f * 0.5f) + (m_fTimer * 3.f);
...
void CAppearRock::Late_Tick(_float fTimeDelta)
{
...
HRESULT CAppearRock::Add_Components()
{
	/* Com_Renderer */
	...
	/* Com_Texture */
	if (FAILED(__super::Add_Component(g_CurLevel, TEXT("Prototype_Component_Texture_Rock"),
...
HRESULT CAppearRock::Bind_ShaderResources()
{
...
CAppearRock* CAppearRock::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CAppearRock* pInstance = new CAppearRock(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Created : CAppearRock");
	}

	return pInstance;
}

CGameObject* CAppearRock::Clone(void* pArg)
{
	CAppearRock* pInstance = new CAppearRock(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Cloned : CAppearRock");
	}

	return pInstance;
}

void CAppearRock::Free()
{
	__super::Free();
