#include "stdafx.h"
#include "..\Public\Thresher_MeleeCollider.h"
...
CThresher_MeleeCollider::CThresher_MeleeCollider(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CThresher_MeleeCollider::CThresher_MeleeCollider(const CThresher_MeleeCollider & rhs)
	: CGameObject(rhs)
{
}

HRESULT CThresher_MeleeCollider::Initialize_Prototype()
{
...
HRESULT CThresher_MeleeCollider::Initialize(void * pArg)
{
	...
	g_CollidedMeleeNumber = m_iNumbering = pColliderDesc->Numbering;
...
void CThresher_MeleeCollider::Tick(_float fTimeDelta)
{
	...
	m_pObject = pGameInstance->Get_GameObject(LEVEL_GAMEPLAY, TEXT("Layer_ThresherMelee"));

	m_pArmTransformCom = dynamic_cast<CTransform*>(pGameInstance->Get_Component(LEVEL_GAMEPLAY, TEXT("Layer_ThresherMelee"), TEXT("Com_Transform")));
	...
	if (pGameInstance->Get_DIMouseState(CInput_Device::DIMK_LBUTTON) & 0x8000)
	{
		...
		{
			_uint Number = dynamic_cast<CThresher_Melee*>(m_pObject)->Get_Numbering();
			if (m_iNumbering == Number)
			{
				...
				if (0.3f <= m_fTimer)
				{
					...
					{
						dynamic_cast<CThresher_Melee*>(m_pObject)->Set_ArmsDead(true);
						g_MeleeNumber = 0;
					}
				}
			}
	...
	if (m_bDemage)
	{
		dynamic_cast<CThresher_Melee*>(m_pObject)->Decrease_Hp(1);
		m_bDemage = false;
	}
	/*if (m_bCollision)
	{
...
void CThresher_MeleeCollider::Late_Tick(_float fTimeDelta)
{
...
HRESULT CThresher_MeleeCollider::Render()
{
	...
	_uint Number = g_MeleeNumber = dynamic_cast<CThresher_Melee*>(m_pObject)->Get_Numbering();

	//pGameInstance->Render_Font(TEXT("Font_136"), to_wstring(Number), _float2(100.f, 50.f), XMVectorSet(0.f, 0.f, 1.f, 1.f), 1.f);
	...
	if (pGameInstance->Get_DIMouseState(CInput_Device::DIMK_RBUTTON) & 0x8000)
	{
		if (m_pColliderCom[COLLIDER_SPHERE]->Collision_Mouse())
		{
			g_bPsychoCollidedMouse3 = true;

			pGameInstance->Render_Font(TEXT("Font_136"), to_wstring(m_iNumbering), _float2(110.f, 100.f), XMVectorSet(0.f, 0.f, 1.f, 1.f), 1.f);
		}
		else
			g_bPsychoCollidedMouse3 = false;
...
HRESULT CThresher_MeleeCollider::Add_Components()
{
	/* Com_Renderer */
...
HRESULT CThresher_MeleeCollider::Bind_ShaderResources()
{
...
HRESULT CThresher_MeleeCollider::Collision_Object()
{
	CGameInstance* pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
...
CThresher_MeleeCollider * CThresher_MeleeCollider::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CThresher_MeleeCollider*	pInstance = new CThresher_MeleeCollider(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Created : CThresher_MeleeCollider");
	}

	return pInstance;
}

CGameObject * CThresher_MeleeCollider::Clone(void * pArg)
{
	CThresher_MeleeCollider*	pInstance = new CThresher_MeleeCollider(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Cloned : CThresher_MeleeCollider");
	}

	return pInstance;
}

void CThresher_MeleeCollider::Free()
{
	__super::Free();
