#include "stdafx.h"
#include "..\Public\Thresher_Arm2Collider.h"
...
CThresher_Arm2Collider::CThresher_Arm2Collider(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CThresher_Arm2Collider::CThresher_Arm2Collider(const CThresher_Arm2Collider & rhs)
	: CGameObject(rhs)
{
}

HRESULT CThresher_Arm2Collider::Initialize_Prototype()
{
...
HRESULT CThresher_Arm2Collider::Initialize(void * pArg)
{
	...
	g_CollidedArm2Number = m_iNumbering = pColliderDesc->Numbering;
...
void CThresher_Arm2Collider::Tick(_float fTimeDelta)
{
	...
	m_pObject = pGameInstance->Get_GameObject(LEVEL_GAMEPLAY, TEXT("Layer_ThresherArm2"));

	m_pArmTransformCom = dynamic_cast<CTransform*>(pGameInstance->Get_Component(LEVEL_GAMEPLAY, TEXT("Layer_ThresherArm2"), TEXT("Com_Transform")));
	...
	if (pGameInstance->Get_DIMouseState(CInput_Device::DIMK_LBUTTON) & 0x8000)
	{
		...
		{
			_uint Number = dynamic_cast<CThresher_Arm2*>(m_pObject)->Get_Numbering();
			if (m_iNumbering == Number)
			{
				...
				if (0.3f <= m_fTimer)
				{
					...
					{
						dynamic_cast<CThresher_Arm2*>(m_pObject)->Set_ArmsDead(true);
						g_Arm2Number = 0;
					}
				}
			}
	...
	if (m_bDemage)
	{
		dynamic_cast<CThresher_Arm2*>(m_pObject)->Decrease_Hp(1);
		m_bDemage = false;
	}
	/*if (m_bCollision)
	{
...
void CThresher_Arm2Collider::Late_Tick(_float fTimeDelta)
{
...
HRESULT CThresher_Arm2Collider::Render()
{
	...
	_int Number = g_Arm2Number = dynamic_cast<CThresher_Arm2*>(m_pObject)->Get_Numbering();

	//pGameInstance->Render_Font(TEXT("Font_136"), to_wstring(Number), _float2(75.f, 50.f), XMVectorSet(0.f, 1.f, 0.f, 1.f), 1.f);
	...
	if (pGameInstance->Get_DIMouseState(CInput_Device::DIMK_RBUTTON) & 0x8000)
	{
		if (m_pColliderCom[COLLIDER_SPHERE]->Collision_Mouse())
		{
			...
			pGameInstance->Render_Font(TEXT("Font_136"), to_wstring(m_iNumbering), _float2(80.f, 100.f), XMVectorSet(0.f, 1.f, 0.f, 1.f), 1.f);
		}
		else
...
HRESULT CThresher_Arm2Collider::Add_Components()
{
	/* Com_Renderer */
...
HRESULT CThresher_Arm2Collider::Bind_ShaderResources()
{
...
HRESULT CThresher_Arm2Collider::Collision_Object()
{
	CGameInstance* pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
...
CThresher_Arm2Collider * CThresher_Arm2Collider::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CThresher_Arm2Collider*	pInstance = new CThresher_Arm2Collider(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Created : CThresher_Arm2Collider");
	}

	return pInstance;
}

CGameObject * CThresher_Arm2Collider::Clone(void * pArg)
{
	CThresher_Arm2Collider*	pInstance = new CThresher_Arm2Collider(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Cloned : CThresher_Arm2Collider");
	}

	return pInstance;
}

void CThresher_Arm2Collider::Free()
{
	__super::Free();
