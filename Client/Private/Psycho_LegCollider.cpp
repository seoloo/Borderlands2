#include "stdafx.h"
#include "..\Public\Psycho_LegCollider.h"

#include "GameInstance.h"

CPsycho_LegCollider::CPsycho_LegCollider(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CPsycho_LegCollider::CPsycho_LegCollider(const CPsycho_LegCollider & rhs)
	: CGameObject(rhs)
{
}

HRESULT CPsycho_LegCollider::Initialize_Prototype()
{
...
HRESULT CPsycho_LegCollider::Initialize(void * pArg)
{
...
void CPsycho_LegCollider::Tick(_float fTimeDelta)
{
	...
	/*if (pGameInstance->Get_DIMouseState(CInput_Device::DIMK_LBUTTON) & 0x8000)
	{
		...
		if (0.3f <= m_fTimer)
		{
				...
				m_pColliderCom[COLLIDER_SPHERE]->Collision_Mouse())
			{
				...
				if (m_iNumber == 1)
					m_pObjectTransformCom = dynamic_cast<CTransform*>(pGameInstance->Get_Component(LEVEL_GAMEPLAY, TEXT("Layer_Psycho"), TEXT("Com_Transform")));
				else if (m_iNumber == 2)
					m_pObjectTransformCom = dynamic_cast<CTransform*>(pGameInstance->Get_Component(LEVEL_GAMEPLAY, TEXT("Layer_Psycho2"), TEXT("Com_Transform")));
				else if (m_iNumber == 3)
					m_pObjectTransformCom = dynamic_cast<CTransform*>(pGameInstance->Get_Component(LEVEL_GAMEPLAY, TEXT("Layer_Psycho3"), TEXT("Com_Transform")));
				...
				while (true)
				{
					...
					Desc.vPivotPos.x += m_pObjectTransformCom->Get_State(CTransform::STATE_LOOK).m128_f32[0];
					Desc.vPivotPos.z += m_pObjectTransformCom->Get_State(CTransform::STATE_LOOK).m128_f32[2];
...
void CPsycho_LegCollider::Late_Tick(_float fTimeDelta)
{
	__super::Late_Tick(fTimeDelta);
}

HRESULT CPsycho_LegCollider::Render()
{
...
HRESULT CPsycho_LegCollider::Add_Components()
{
	/* Com_Renderer */
	...
	ColliderDesc.vSize = _float3(55.0f, 150.0f, 55.0f);
...
HRESULT CPsycho_LegCollider::Bind_ShaderResources()
{
...
CPsycho_LegCollider * CPsycho_LegCollider::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CPsycho_LegCollider*	pInstance = new CPsycho_LegCollider(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Created : CPsycho_LegCollider");
	}

	return pInstance;
}

CGameObject * CPsycho_LegCollider::Clone(void * pArg)
{
	CPsycho_LegCollider*	pInstance = new CPsycho_LegCollider(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Cloned : CPsycho_LegCollider");
	}

	return pInstance;
}

void CPsycho_LegCollider::Free()
{
	__super::Free();
