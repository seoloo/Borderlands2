...
HRESULT CMetalBeam::Add_Components()
{
	...
	/* Com_Model */
	if (FAILED(__super::Add_Component(g_CurLevel, TEXT("Prototype_Component_Model_MetalBeam"),
...
CMetalBeam* CMetalBeam::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	...
	if (FAILED(pInstance->Initialize_Prototype()))
	{
		...
		MSG_BOX("Failed to Created : CMetalBeam");
	}

	return pInstance;
}

CGameObject* CMetalBeam::Clone(void* pArg)
{
	...
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Cloned : CMetalBeam");
	}
