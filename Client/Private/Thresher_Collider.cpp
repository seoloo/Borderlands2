#include "stdafx.h"
#include "..\Public\Thresher_Collider.h"

#include "GameInstance.h"

CThresher_Collider::CThresher_Collider(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CThresher_Collider::CThresher_Collider(const CThresher_Collider & rhs)
	: CGameObject(rhs)
{
}

HRESULT CThresher_Collider::Initialize_Prototype()
{
...
HRESULT CThresher_Collider::Initialize(void * pArg)
{
...
void CThresher_Collider::Tick(_float fTimeDelta)
{
	...
	if (m_bDemage)
	{
		random_device rd;
		mt19937 gen(rd());
		uniform_int_distribution<int> Demage(30, 35);
...
void CThresher_Collider::Late_Tick(_float fTimeDelta)
{
	__super::Late_Tick(fTimeDelta);

}

HRESULT CThresher_Collider::Render()
{
...
HRESULT CThresher_Collider::Add_Components()
{
	/* Com_Renderer */
...
HRESULT CThresher_Collider::Bind_ShaderResources()
{
...
CThresher_Collider * CThresher_Collider::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CThresher_Collider*	pInstance = new CThresher_Collider(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Created : CThresher_Collider");
	}

	return pInstance;
}

CGameObject * CThresher_Collider::Clone(void * pArg)
{
	CThresher_Collider*	pInstance = new CThresher_Collider(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Cloned : CThresher_Collider");
	}

	return pInstance;
}

void CThresher_Collider::Free()
{
	__super::Free();
