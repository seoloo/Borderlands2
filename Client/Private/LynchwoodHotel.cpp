...
HRESULT CLynchwoodHotel::Add_Components()
{
	...
	/* Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_LynchwoodHotel"),
...
CLynchwoodHotel* CLynchwoodHotel::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	...
	if (FAILED(pInstance->Initialize_Prototype()))
	{
		...
		MSG_BOX("Failed to Created : CLynchwoodHotel");
	}

	return pInstance;
}

CGameObject* CLynchwoodHotel::Clone(void* pArg)
{
	...
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Cloned : CLynchwoodHotel");
	}
