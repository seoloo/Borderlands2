#include "stdafx.h"
#include "..\Public\SpiderAnt_LegCollider.h"

#include "GameInstance.h"

#include "Player.h"

CSpiderAnt_LegCollider::CSpiderAnt_LegCollider(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CSpiderAnt_LegCollider::CSpiderAnt_LegCollider(const CSpiderAnt_LegCollider & rhs)
	: CGameObject(rhs)
{
}

HRESULT CSpiderAnt_LegCollider::Initialize_Prototype()
{
...
HRESULT CSpiderAnt_LegCollider::Initialize(void * pArg)
{
...
void CSpiderAnt_LegCollider::Tick(_float fTimeDelta)
{
...
void CSpiderAnt_LegCollider::Late_Tick(_float fTimeDelta)
{
...
HRESULT CSpiderAnt_LegCollider::Render()
{
...
HRESULT CSpiderAnt_LegCollider::Add_Components()
{
	/* Com_Renderer */
	...
	ColliderDesc.vSize = _float3(135.0f, 125.0f, 135.0f);
...
HRESULT CSpiderAnt_LegCollider::Bind_ShaderResources()
{
...
HRESULT CSpiderAnt_LegCollider::Collision_Object()
{
	CGameInstance* pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
...
CSpiderAnt_LegCollider * CSpiderAnt_LegCollider::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CSpiderAnt_LegCollider*	pInstance = new CSpiderAnt_LegCollider(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Created : CSpiderAnt_LegCollider");
	}

	return pInstance;
}

CGameObject * CSpiderAnt_LegCollider::Clone(void * pArg)
{
	CSpiderAnt_LegCollider*	pInstance = new CSpiderAnt_LegCollider(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Cloned : CSpiderAnt_LegCollider");
	}

	return pInstance;
}

void CSpiderAnt_LegCollider::Free()
{
	__super::Free();
