#include "stdafx.h"
#include "..\Public\Skag_Collider.h"

#include "GameInstance.h"
#include "Player.h"

CSkag_Collider::CSkag_Collider(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CSkag_Collider::CSkag_Collider(const CSkag_Collider & rhs)
	: CGameObject(rhs)
{
}

HRESULT CSkag_Collider::Initialize_Prototype()
{
...
HRESULT CSkag_Collider::Initialize(void * pArg)
{
...
void CSkag_Collider::Tick(_float fTimeDelta)
{
...
void CSkag_Collider::Late_Tick(_float fTimeDelta)
{
...
HRESULT CSkag_Collider::Add_Components()
{
	/* Com_Renderer */
	...
	ColliderDesc.vSize = _float3(125.0f, 75.0f, 125.0f);
...
HRESULT CSkag_Collider::Bind_ShaderResources()
{
...
HRESULT CSkag_Collider::Collision_Object()
{
	CGameInstance* pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
	...
	if (m_pColliderCom[COLLIDER_AABB]->Collision((CCollider*)pGameInstance->Get_Component(LEVEL_GAMEPLAY, TEXT("Layer_Player"), TEXT("Com_OBB")))||
		m_pColliderCom[COLLIDER_SPHERE]->Collision((CCollider*)pGameInstance->Get_Component(LEVEL_GAMEPLAY, TEXT("Layer_Player"), TEXT("Com_OBB")))) {
		m_bCollision = true;
	}
	else
		m_bCollision = false;
...
CSkag_Collider * CSkag_Collider::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CSkag_Collider*	pInstance = new CSkag_Collider(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Created : CSkag_Collider");
	}

	return pInstance;
}

CGameObject * CSkag_Collider::Clone(void * pArg)
{
	CSkag_Collider*	pInstance = new CSkag_Collider(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Cloned : CSkag_Collider");
	}

	return pInstance;
}

void CSkag_Collider::Free()
{
	__super::Free();
