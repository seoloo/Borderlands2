...
CEMPRed::CEMPRed(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

CEMPRed::CEMPRed(const CEMPRed& rhs)
	:CGameObject(rhs)
{
}

HRESULT CEMPRed::Initialize_Prototype()
{
...
HRESULT CEMPRed::Initialize(void* pArg)
{
...
void CEMPRed::Tick(_float fTimeDelta)
{
...
void CEMPRed::Late_Tick(_float fTimeDelta)
{
...
HRESULT CEMPRed::Add_Components()
{
	/* Com_Renderer */
	...
	/* Com_Texture */
	if (FAILED(__super::Add_Component(g_CurLevel, TEXT("Prototype_Component_Texture_EMPRed"),
...
HRESULT CEMPRed::Bind_ShaderResources()
{
...
HRESULT CEMPRed::Collision_Object()
{
	CGameInstance* pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
...
CEMPRed* CEMPRed::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEMPRed* pInstance = new CEMPRed(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Created : CEMPRed");
	}

	return pInstance;
}

CGameObject* CEMPRed::Clone(void* pArg)
{
	CEMPRed* pInstance = new CEMPRed(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Cloned : CEMPRed");
	}
