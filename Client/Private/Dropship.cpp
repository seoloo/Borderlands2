...
CDropship::CDropship(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CGameObject(pDevice, pContext)
{
}

CDropship::CDropship(const CDropship& rhs)
    : CGameObject(rhs)
{
}

HRESULT CDropship::Initialize_Prototype()
{
...
HRESULT CDropship::Initialize(void* pArg)
{
...
void CDropship::Tick(_float fTimeDelta)
{
    __super::Tick(fTimeDelta);
}

void CDropship::Late_Tick(_float fTimeDelta)
{
	...
	if (true == pGameInstance->isInFrustum_InWorld(m_pTransformCom->Get_State(CTransform::STATE_POSITION), 100.f))
	{
...
HRESULT CDropship::Add_Components()
{
	/* Com_Renderer */
	...
	/* Com_Model */
	if (FAILED(__super::Add_Component(g_CurLevel, TEXT("Prototype_Component_Model_Dropship"),
...
HRESULT CDropship::Bind_ShaderResources()
{
...
CDropship* CDropship::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CDropship* pInstance = new CDropship(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Created : CDropship");
	}

	return pInstance;
}

CGameObject* CDropship::Clone(void* pArg)
{
	CDropship* pInstance = new CDropship(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Cloned : CDropship");
	}
