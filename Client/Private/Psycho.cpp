...
CPsycho::CPsycho(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CLandObject(pDevice, pContext)
{
}

CPsycho::CPsycho(const CPsycho & rhs)
	: CLandObject(rhs)
{
}

HRESULT CPsycho::Initialize_Prototype()
{
...
HRESULT CPsycho::Initialize(void * pArg)
{
	...
	if (Desc->iNumber == 1)
	{
		m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMVectorSet(35.f, 0.f, 105.f, 1.f));
		m_iNumber = 1;
	}
	else if (Desc->iNumber == 2)
	{
		m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMVectorSet(35.f, 0.f, 50.f, 1.f));
		m_iNumber = 2;
	}
	else if (Desc->iNumber == 3)
	{
		m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMVectorSet(45.f, 0.f, 80.f, 1.f));
		m_iNumber = 3;
	}
...
void CPsycho::Tick(_float fTimeDelta)
{
...
void CPsycho::Late_Tick(_float fTimeDelta)
{
...
void CPsycho::Idle_Mode(_float fTimeDelta)
{
...
void CPsycho::Appear_Mode(_float fTimeDelta)
{
	m_pModelCom->Play_Animation(fTimeDelta);
...
void CPsycho::Walk_Mode(_float fTimeDelta)
{
	CGameInstance* pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
	m_pModelCom->Play_Animation(fTimeDelta);
	...
	if (m_MoveState == MOVE_FORWARD)
	{
		...
		if (m_fTimer >= 1.f)
		{
			m_vCurrentPos = m_pTransformCom->Get_State(CTransform::STATE_POSITION);

			m_fTimer = 0.f;

			if (m_vCurrentPos.m128_f32[0] == m_vPreviousPos.m128_f32[0] &&
				m_vCurrentPos.m128_f32[2] == m_vPreviousPos.m128_f32[2])
	...
	{
		m_pTransformCom->Go_Backward(fTimeDelta * 5.f, m_pNavigationCom);
		m_MoveState = MOVE_RIGHT;
	}

	if (m_MoveState == MOVE_RIGHT)
	{
		m_pTransformCom->Go_Right(fTimeDelta * 5.f, m_pNavigationCom);
		m_MoveState = MOVE_FORWARD;
	}
...
void CPsycho::Attack_Mode(_float fTimeDelta)
{
	m_pModelCom->Play_Animation(fTimeDelta);
...
void CPsycho::Attacked_Mode(_float fTimeDelta)
{
	m_pModelCom->Play_Animation(fTimeDelta);
...
void CPsycho::Dead_Mode(_float fTimeDelta)
{
	m_pModelCom->Play_Animation(fTimeDelta);
...
HRESULT CPsycho::Add_Components()
{
	/* Com_Renderer */
...
HRESULT CPsycho::Bind_ShaderResources()
{
...
HRESULT CPsycho::Collision_Object()
{
	CGameInstance* pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
...
CPsycho * CPsycho::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CPsycho*	pInstance = new CPsycho(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Created : CPsycho");
	}

	return pInstance;
}

CGameObject * CPsycho::Clone(void * pArg)
{
	CPsycho*	pInstance = new CPsycho(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Cloned : CPsycho");
	}
