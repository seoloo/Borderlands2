...
CEMPBlue::CEMPBlue(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

CEMPBlue::CEMPBlue(const CEMPBlue& rhs)
	:CGameObject(rhs)
{
}

HRESULT CEMPBlue::Initialize_Prototype()
{
...
HRESULT CEMPBlue::Initialize(void* pArg)
{
	...
	m_Desc.fLifeTime = 1;
	//m_Desc.vPivotPos = _float4(10.f, 5.f, 10.f, 0.f);
	...
	m_pObjectTransformCom = dynamic_cast<CTransform*>(pGameInstance->Get_Component(LEVEL_GAMEPLAY, TEXT("Layer_SpiderAnt"), TEXT("Com_Transform")));
	...
	m_vLook = m_pPlayerTransformCom->Get_State(CTransform::STATE_POSITION) - XMVectorSet(m_fPos.x, m_fPos.y, m_fPos.z, 1.f);

	CSoundMgr::Get_Instance()->PlaySoundW(L"BossCav - Pattern_1_Fire.wav", 1.f);
...
void CEMPBlue::Tick(_float fTimeDelta)
{
...
void CEMPBlue::Late_Tick(_float fTimeDelta)
{
...
HRESULT CEMPBlue::Add_Components()
{
	/* Com_Renderer */
	...
	/* Com_Texture */
	if (FAILED(__super::Add_Component(g_CurLevel, TEXT("Prototype_Component_Texture_EMPBlue"),
...
HRESULT CEMPBlue::Bind_ShaderResources()
{
...
HRESULT CEMPBlue::Collision_Object()
{
	CGameInstance* pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
...
CEMPBlue* CEMPBlue::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEMPBlue* pInstance = new CEMPBlue(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Created : CEMPBlue");
	}

	return pInstance;
}

CGameObject* CEMPBlue::Clone(void* pArg)
{
	CEMPBlue* pInstance = new CEMPBlue(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Cloned : CEMPBlue");
	}
