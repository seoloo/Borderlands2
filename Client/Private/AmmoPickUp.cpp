#include "stdafx.h"
#include "..\Public\AmmoPickUp.h"

#include "GameInstance.h"

#include "Player.h"

CAmmoPickUp::CAmmoPickUp(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CAmmoPickUp::CAmmoPickUp(const CAmmoPickUp & rhs)
	: CGameObject(rhs)
{
}

HRESULT CAmmoPickUp::Initialize_Prototype()
{
...
HRESULT CAmmoPickUp::Initialize(void * pArg)
{
	...
	if(pDesc->iNumber == 1)
		m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMVectorSet(-40.f, 0.f, -10.f, 1.f));
	else if(pDesc->iNumber == 2)
		m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMVectorSet(50.f, 0.f, -10.f, 1.f));

	return S_OK;
}

void CAmmoPickUp::Tick(_float fTimeDelta)
{
...
void CAmmoPickUp::Late_Tick(_float fTimeDelta)
{
...
HRESULT CAmmoPickUp::Add_Components()
{
	/* Com_Renderer */
	...
	/* Com_Model */
	if (FAILED(__super::Add_Component(g_CurLevel, TEXT("Prototype_Component_Model_AmmoPickUp"),
...
HRESULT CAmmoPickUp::Bind_ShaderResources()
{
...
HRESULT CAmmoPickUp::Collision_Object()
{
	CGameInstance* pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
...
CAmmoPickUp * CAmmoPickUp::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CAmmoPickUp*	pInstance = new CAmmoPickUp(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Created : CAmmoPickUp");
	}

	return pInstance;
}

CGameObject * CAmmoPickUp::Clone(void * pArg)
{
	CAmmoPickUp*	pInstance = new CAmmoPickUp(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Cloned : CAmmoPickUp");
	}

	return pInstance;
}

void CAmmoPickUp::Free()
{
	__super::Free();
